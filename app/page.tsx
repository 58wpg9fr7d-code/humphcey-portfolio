...
export default function Home() {
  ...
  return (
    <main id="top">
      ...
      <section className="case shell" id="case" aria-live="polite"><div className="case-visual"><div className={`visual-frame ${selected.visual} ${selected.embed ? "embedded-demo" : ""}`} key={selected.id}>{selected.id === "jingye" ? <JingyeGuide /> : selected.embed ? <iframe src={selected.embed} title={`${selected.name}可滚动交互 Demo`} loading="lazy" /> : selected.gallery ? <div className="workbench-gallery">{selected.gallery.map((image) => <img key={image} src={image} alt={`${selected.name}页面截图`} />)}</div> : selected.image ? <img src={selected.image} alt={`${selected.name} project`} /> : selected.visual === "weread" ? <><div className="mock-browser"><span className="mock-dots" /><b>微信读书</b><span className="mock-search">⌕</span></div><div className="mock-book">百年孤独<small>阅读回顾</small></div><div className="mock-side"><small>阅读记忆</small><strong>上次读到<br />第 7 章</strong><span>查看关键情节　↗</span></div></> : <><div className="mock-board"><span>AI 收件箱</span><b>今天要做的事</b><i>确认后生成任务</i><i>学习 02:30:00</i></div><div className="mock-orbit" /></>}</div><span className="visual-caption">{selected.name} / {selected.embed ? "可滚动交互 Demo" : selected.id === "jingye" ? "操作向导式互动预览" : selected.gallery ? "四个页面截图" : selected.image ? "真实产品页面" : "真实 Demo 预览"}</span></div><div className="case-copy"><div className="case-meta"><span>{selected.no} / {selected.category}</span><span>项目拆解</span></div><h2>{selected.summary}</h2><div className="case-section"><span>独立完成</span><p>{selected.role}</p></div><div className="case-section"><span>体验结构</span><p>{selected.shipped}</p></div>{selected.id === "jingye" && <div className="case-section"><span>设计判断</span><p>先让玩家进入角色，再按幕次释放信息；把复杂叙事拆成可操作的界面状态。AI 用于辅助视觉生成和素材迭代，产品结构与交互判断由本人完成。</p></div>}<div className="case-tags">{selected.skills.map((skill) => <span key={skill}>{skill}</span>)}</div><div className="case-actions"><a href={selected.demo} target="_blank" rel="noreferrer">打开 Demo <span>↗</span></a>{selected.source && <a href={selected.source} target="_blank" rel="noreferrer">查看源码 <span>↗</span></a>}</div></div></section>
