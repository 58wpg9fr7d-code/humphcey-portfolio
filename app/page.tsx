...
export default function Home() {
  ...
  return (
    <main id="top">
      ...
      <section className="case shell"><div className="case-visual"><div className={`visual-frame ${selected.visual}`}>{selected.image ? <img src={selected.image} alt={`${selected.name} project`} /> : selected.embed ? <iframe src={selected.embed} title={`${selected.name}真实 Demo`} loading="lazy" /> : selected.visual === "weread" ? <><div className="mock-browser"><span className="mock-dots" /><b>微信读书</b><span className="mock-search">⌕</span></div><div className="mock-book">百年孤独<small>阅读回顾</small></div><div className="mock-side"><small>阅读记忆</small><strong>上次读到<br />第 7 章</strong><span>查看关键情节　↗</span></div></> : <><div className="mock-board"><span>AI 收件箱</span><b>今天要做的事</b><i>确认后生成任务</i><i>学习 02:30:00</i></div><div className="mock-orbit" /></>}</div><span className="visual-caption">{selected.name} / {selected.id === "workbench" || selected.id === "weread" ? "真实 Demo 预览" : selected.id === "jingye" ? "游戏封面与角色设定" : "产品展示"}</span>{selected.id === "jingye" && <div className="character-strip"><figure><img src="/projects/jingye/niu-shouzhuo-folder.png" alt="角色设定图" /></figure><figure><img src="/projects/jingye/hu-mou-folder.png" alt="角色设定图" /></figure><figure><img src="/projects/jingye/zhu-ke-yan-folder.png" alt="角色设定图" /></figure><figure><img src="/projects/jingye/zhang-mo-folder.png" alt="角色设定图" /></figure><figure><img src="/projects/jingye/xiang-chen-folder.png" alt="角色设定图" /></figure></div>}</div><div className="case-copy"><div className="case-meta"><span>{selected.no} / {selected.category}</span><span>项目拆解</span></div><h2>{selected.summary}</h2><div className="case-section"><span>我负责的部分</span><p>{selected.role}</p></div><div className="case-section"><span>体验结构</span><p>{selected.shipped}</p></div>{selected.id === "jingye" && <div className="case-section"><span>设计判断</span><p>先让玩家进入角色，再按幕次释放信息；把复杂叙事拆成可操作的界面状态。AI 用于辅助视觉生成和素材迭代，产品结构与交互判断由本人完成。</p></div>}<div className="case-tags">{selected.skills.map((skill) => <span key={skill}>{skill}</span>)}</div><div className="case-actions"><a href={selected.demo} target="_blank" rel="noreferrer">打开 Demo <span>↗</span></a>{selected.source && <a href={selected.source} target="_blank" rel="noreferrer">查看源码 <span>↗</span></a>}</div></div></section>
      ...
      <section className="contact shell" id="contact"><div><span className="section-line-label">04 / 联系方式</span><h2>有想法，<br /><span>欢迎一起聊聊。</span></h2></div><div className="contact-note"><p>正在寻找人工智能产品经理实习机会<br />杭州 · 立即到岗 · 每周 5 天</p><div className="contact-details"><span>电话　134 7801 4055</span><span>邮箱　humphcey@163.com</span><span>微信　简历未提供</span></div><a href="#top">回到顶部 ↑</a></div></section>
