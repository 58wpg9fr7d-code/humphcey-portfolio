...
const projects: Project[] = [
  ...
  { id: "weread", no: "02", name: "WeRead AI", category: "浏览器插件 / 本地工具", year: "2026", summary: "把阅读回顾和笔记沉淀放回阅读现场。", role: "产品设计、本地工具工作流", shipped: "情节节点、阅读回顾、划线整理、Obsidian 导出", skills: ["场景建模", "插件架构", "状态设计"], demo: "https://58wpg9fr7d-code.github.io/weread-ai-reading-assistant/", source: "https://github.com/58wpg9fr7d-code/weread-ai-reading-assistant", image: "/projects/weread-concept.png", visual: "weread" },
  { id: "jingye", no: "03", name: "静夜园", category: "叙事系统 / 最小可用版本", year: "2026", summary: "把复杂剧情拆成玩家看得懂、做得到的选择。", role: "世界观、信息架构、交互与网页 MVP", shipped: "角色分级、线索解锁、线索板、手机端剧情入口", skills: ["叙事结构", "信息分层", "视觉系统"], demo: "https://murder-mystery-studio.vercel.app/phone", image: "/projects/jingye-cover-site.jpeg", visual: "jingye" },
  { id: "workbench", no: "04", name: "个人 AI 工作台", category: "Agent 工作流 / 个人系统", year: "2026", summary: "让 AI 建议经过确认，才变成今天真正要做的事。", role: "产品架构、交互设计、脱敏 Demo", shipped: "AI 收件箱、今日安排、学习计时、完成条件", skills: ["Agent 边界", "工作流设计", "产品状态"], demo: "https://58wpg9fr7d-code.github.io/personal-ai-workbench-demo/", source: "https://github.com/58wpg9fr7d-code/personal-ai-workbench-demo", image: "/projects/workbench-concept.png", visual: "workbench" }
];
...
export default function Home() {
  ...
  return (
    <main id="top">
      ...
      <section className="case shell"><div className="case-visual"><div className={`visual-frame ${selected.visual}`}>{selected.image ? <img src={selected.image} alt={`${selected.name} project`} /> : selected.visual === "weread" ? <><div className="mock-browser"><span className="mock-dots" /><b>微信读书</b><span className="mock-search">⌕</span></div><div className="mock-book">百年孤独<small>阅读回顾</small></div><div className="mock-side"><small>阅读记忆</small><strong>上次读到<br />第 7 章</strong><span>查看关键情节　↗</span></div></> : <><div className="mock-board"><span>AI 收件箱</span><b>今天要做的事</b><i>确认后生成任务</i><i>学习 02:30:00</i></div><div className="mock-orbit" /></>}</div><span className="visual-caption">{selected.name} / {selected.id === "weread" || selected.id === "workbench" ? "产品概念视觉" : "游戏封面与角色设定"}</span>{selected.id === "jingye" && <div className="character-strip"><figure><img src="/projects/jingye/niu-shouzhuo-folder.png" alt="牛守拙角色设定" /><figcaption>牛守拙 / 观察者</figcaption></figure><figure><img src="/projects/jingye/hu-mou-folder.png" alt="胡谋角色设定" /><figcaption>胡谋 / 叙事线索</figcaption></figure><figure><img src="/projects/jingye/zhu-ke-yan-folder.png" alt="朱渴焰角色设定" /><figcaption>朱渴焰 / 关键角色</figcaption></figure><figure><img src="/projects/jingye/zhang-mo-folder.png" alt="章貘角色设定" /><figcaption>章貘 / 角色设定</figcaption></figure><figure><img src="/projects/jingye/xiang-chen-folder.png" alt="向沉角色设定" /><figcaption>向沉 / 角色设定</figcaption></figure></div>}</div><div className="case-copy"><div className="case-meta"><span>{selected.no} / {selected.category}</span><span>项目拆解</span></div><h2>{selected.summary}</h2><div className="case-section"><span>我的角色</span><p>{selected.role}</p></div><div className="case-section"><span>已完成</span><p>{selected.shipped}</p></div>{selected.id === "jingye" && <div className="case-section"><span>设计方法</span><p>以角色、幕次和剧情阶段组织信息，通过分层解锁控制节奏。AI 用于辅助视觉生成、氛围建立和素材迭代，产品结构与交互判断由本人完成。</p></div>}<div className="case-tags">{selected.skills.map((skill) => <span key={skill}>{skill}</span>)}</div><div className="case-actions"><a href={selected.demo} target="_blank" rel="noreferrer">打开 Demo <span>↗</span></a>{selected.source && <a href={selected.source} target="_blank" rel="noreferrer">查看源码 <span>↗</span></a>}</div></div></section>
