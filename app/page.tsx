...
export default function Home() {
  ...
  return (
    <main>
      ...
      <section className="architecture shell"><div className="section-line"><span>03 / BEFORE AI</span><span>建筑训练与视觉表达</span></div><div className="architecture-intro"><div><h2>从空间开始，<br />练习处理复杂约束。</h2></div><p>建筑学习让我长期处理场地、功能、动线、尺度和视觉表达。后来做 AI 产品，这种训练转成了对信息结构、用户路径和交互节奏的敏感。</p></div><div className="architecture-grid"><figure className="architecture-feature"><img src="/projects/architecture/biodirectional-render.png" alt="流浪动物公园建筑效果图" /><figcaption><strong>流浪动物公园</strong><span>把公共空间和动物救助组织在同一条体验路径里。</span></figcaption></figure><figure><img src="/projects/architecture/mine-restoration-render.png" alt="矿山修复工程建筑效果图" /><figcaption><strong>矿山修复工程</strong><span>从环境问题出发组织功能、景观和时间变化。</span></figcaption></figure><figure><img src="/projects/architecture/painting-works.png" alt="绘画与手绘作品" /><figcaption><strong>绘画与手绘</strong><span>保留观察、构图和氛围表达。</span></figcaption></figure></div></section>
      <section className="profile shell" id="profile"><div className="section-line"><span>04 / ABOUT</span><span>一个建筑背景的 AI 产品人</span></div><div className="profile-grid"><div><h2>我擅长把<em>复杂信息理顺。</em></h2></div><div className="profile-content"><p className="profile-lead">建筑训练让我习惯处理复杂约束。做 AI 产品后，我把这种能力用在需求拆解、信息架构、模型边界和原型验证上。</p><div className="profile-facts">{profile.map((item) => <div key={item}><i />{item}</div>)}</div><div className="skills"><span>PRODUCT</span><p>用户问题 · 需求拆解 · MVP · PRD · 信息架构 · 用户流程 · 原型设计</p><span>AI APPLICATION</span><p>Prompt · RAG · Agent 基础 · 输出评测 · 人工校正</p><span>BUILD</span><p>Python · TypeScript · React · Next.js · FastAPI · GitHub</p></div></div></div></section>

      <section className="contact shell" id="contact"><div><span className="section-line-label">05 / CONTACT</span><h2>有问题，<span>一起把它做出来。</span></h2></div><div className="contact-note"><p>正在寻找 AI 产品经理实习机会，杭州，立即到岗，每周 5 天。</p><a href="#top">回到顶部 ↑</a></div></section>
