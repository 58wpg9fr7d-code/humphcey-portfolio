...
const projects: Project[] = [
  ...
  { id: "jingye", no: "03", name: "静夜园", category: "叙事系统 / 最小可用版本", year: "2026", summary: "把复杂剧情拆成玩家看得懂、做得到的选择。", role: "世界观、信息架构、交互与网页 MVP", shipped: "角色分级、线索解锁、线索板、手机端剧情入口", skills: ["叙事结构", "信息分层", "视觉系统"], demo: "https://murder-mystery-studio.vercel.app/phone", image: "/projects/jingye-cover-real.png", visual: "jingye" },
...
export default function Home() {
  ...
  return (
    <main id="top">
      ...
      <section className="case shell"><div className="case-visual"><div className={`visual-frame ${selected.visual}`}>{selected.image ? <img src={selected.image} alt={`${selected.name} project`} /> : selected.visual === "weread" ? <><div className="mock-browser"><span className="mock-dots" /><b>微信读书</b><span className="mock-search">⌕</span></div><div className="mock-book">百年孤独<small>阅读回顾</small></div><div className="mock-side"><small>阅读记忆</small><strong>上次读到<br />第 7 章</strong><span>查看关键情节　↗</span></div></> : <><div className="mock-board"><span>AI 收件箱</span><b>今天要做的事</b><i>确认后生成任务</i><i>学习 02:30:00</i></div><div className="mock-orbit" /></>}</div><span className="visual-caption">{selected.name} / {selected.id === "weread" || selected.id === "workbench" ? "产品概念视觉" : "产品展示"}</span>{selected.id === "jingye" && <div className="character-strip"><figure><img src="/projects/jingye/niu-shouzhuo-real.png" alt="牛守拙角色设定" /><figcaption>牛守拙 / 观察者</figcaption></figure><figure><img src="/projects/jingye/hu-mou-real.png" alt="胡某角色设定" /><figcaption>胡某 / 叙事线索</figcaption></figure><figure><img src="/projects/jingye/zhu-ke-yan-real.png" alt="朱可言角色设定" /><figcaption>朱可言 / 关键角色</figcaption></figure></div>}</div><div className="case-copy"><div className="case-meta"><span>{selected.no} / {selected.category}</span><span>项目拆解</span></div><h2>{selected.summary}</h2><div className="case-section"><span>我的角色</span><p>{selected.role}</p></div><div className="case-section"><span>已完成</span><p>{selected.shipped}</p></div>{selected.id === "jingye" && <div className="case-section"><span>设计方法</span><p>以角色、幕次和剧情阶段组织信息，通过分层解锁控制节奏。AI 用于辅助视觉生成、氛围建立和素材迭代，产品结构与交互判断由本人完成。</p></div>}<div className="case-tags">{selected.skills.map((skill) => <span key={skill}>{skill}</span>)}</div><div className="case-actions"><a href={selected.demo} target="_blank" rel="noreferrer">打开 Demo <span>↗</span></a>{selected.source && <a href={selected.source} target="_blank" rel="noreferrer">查看源码 <span>↗</span></a>}</div></div></section>
