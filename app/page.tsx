...
export default function Home() {
  ...
  return (
    <main id="top">
      ...
      <section className="case shell"><div className="case-visual"><div className={`visual-frame ${selected.visual}`}>{selected.image ? <img src={selected.image} alt={`${selected.name} project`} /> : selected.embed ? <iframe src={selected.embed} title={`${selected.name}真实 Demo`} loading="lazy" /> : selected.visual === "weread" ? <><div className="mock-browser"><span className="mock-dots" /><b>微信读书</b><span className="mock-search">⌕</span></div><div className="mock-book">百年孤独<small>阅读回顾</small></div><div className="mock-side"><small>阅读记忆</small><strong>上次读到<br />第 7 章</strong><span>查看关键情节　↗</span></div></> : <><div className="mock-board"><span>AI 收件箱</span><b>今天要做的事</b><i>确认后生成任务</i><i>学习 02:30:00</i></div><div className="mock-orbit" /></>}</div><span className="visual-caption">{selected.name} / {selected.id === "workbench" ? "真实 Demo 预览" : selected.id === "weread" ? "产品概念视觉" : "游戏封面与角色设定"}</span>{selected.id === "jingye" && <div className="character-strip"><figure><img src="/projects/jingye/niu-shouzhuo-folder.png" alt="牛守拙角色设定" /><figcaption>牛守拙 / 观察者</figcaption></figure><figure><img src="/projects/jingye/hu-mou-folder.png" alt="胡谋角色设定" /><figcaption>胡谋 / 叙事线索</figcaption></figure><figure><img src="/projects/jingye/zhu-ke-yan-folder.png" alt="朱渴焰角色设定" /><figcaption>朱渴焰 / 关键角色</figcaption></figure><figure><img src="/projects/jingye/zhang-mo-folder.png" alt="章貘角色设定" /><figcaption>章貘 / 角色设定</figcaption></figure><figure><img src="/projects/jingye/xiang-chen-folder.png" alt="向沉角色设定" /><figcaption>向沉 / 角色设定</figcaption></figure></div>}</div><div className="case-copy"><div className="case-meta"><span>{selected.no} / {selected.category}</span><span>项目拆解</span></div><h2>{selected.summary}</h2><div className="case-section"><span>我负责的部分</span><p>{selected.role}</p></div><div className="case-section"><span>体验结构</span><p>{selected.shipped}</p></div>{selected.id === "jingye" && <div className="case-section"><span>设计判断</span><p>先让玩家进入角色，再按幕次释放信息；把复杂叙事拆成可操作的界面状态。AI 用于辅助视觉生成和素材迭代，产品结构与交互判断由本人完成。</p></div>}<div className="case-tags">{selected.skills.map((skill) => <span key={skill}>{skill}</span>)}</div><div className="case-actions"><a href={selected.demo} target="_blank" rel="noreferrer">打开 Demo <span>↗</span></a>{selected.source && <a href={selected.source} target="_blank" rel="noreferrer">查看源码 <span>↗</span></a>}</div></div></section>
